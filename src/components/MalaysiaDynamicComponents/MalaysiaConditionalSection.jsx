...
import { calculateAccommodation, detectHotelType } from "../../utils/invoiceCalculationsMalaysia";
import DatePicker from "../DatePicker";

const MalaysiaConditionalSection = ({ sectionKey, section, formData, onFieldChange, setFormData, hotelConfig }) => {
  ...
  // --------------------------------------------------------------------------
  // MALAYSIA CALCULATION EFFECT
  // Automatically calculates Room Base, SST, and Tourism Tax on the fly
  // --------------------------------------------------------------------------
  useEffect(() => {
    if (sectionKey === 'accommodation_details' && !section.multiple_entries) {
      ...
      if (acc.usd_amount > 0 && acc.exchange_rate > 0) {
       ...
       // Inside MalaysiaConditionalSection.jsx -> useEffect
const updates = {
          // Room Package Base
          nightly_base_myr: calculated.roombaseAmount?.toFixed(2) || "0.00",
          
          // Room SST 8%
          nightly_sst_myr: calculated.roomSST?.toFixed(2) || "0.00",
          
          // Tourism Tax
          nightly_tourism_tax_myr: calculated.tourismTax?.toFixed(2) || "0.00",
          
          // Total Nightly Gross
          room_amount_myr: calculated.totalNightGrossMyr?.toFixed(2) || "0.00",
          
          // Total Room (All Nights MYR)
          total_room_all_nights: calculated.totalRoomAllNightsMyr?.toFixed(2) || "0.00",
        };
...
export default MalaysiaConditionalSection;