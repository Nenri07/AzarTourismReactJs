    ...
    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
      ...
      <div className={section.multiple_entries ? 'space-y-4' : 'grid grid-cols-1 md:grid-cols-3 gap-5'}>
        {section.multiple_entries ? (
            ...
            <div key={entry.id || index} className="p-4 border border-slate-200 rounded-lg bg-slate-50 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {section.fields?.map(f =>
                  renderField(f, entry[f.field_id], (val) => handleEntryFieldChange(index, f.field_id, val), true, index)
                )}
              </div>
            </div>
          ))
          ...
          section.fields?.map(f =>
            renderField(f, sectionData[f.field_id], (val) => onFieldChange(`${sectionKey}.${f.field_id}`, val))
          )
        )}
      </div>
      ...
      {section.multiple_entries && (
        <div className="mt-5 flex justify-end border-t border-slate-100 pt-4">
          ...
          >
            <Plus size={16} /> Add Extra Services
          </button>
        </div>
      )}
    </div>
