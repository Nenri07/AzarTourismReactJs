...
import { getHotelConfigsPaginated, createHotelConfig, updateHotelConfig, deleteHotelConfig, getHotelsByCountryPaginated } from '../Api/hotelConfig.api';
...
const COUNTRIES = ['Turkey', 'Egypt', 'Tunis', 'Malaysia', 'London', 'Germany', 'Qatar', 'Bahrain', 'Oman', 'Kuwait', 'UK', 'Global'];
const LIMIT = 10;
...
const HotelConfigPage = () => {
  ...
  try {
    ...
    if (country) {
      response = await getHotelsByCountryPaginated(country, page, LIMIT);
    } else {
      response = await getHotelConfigsPaginated(page, LIMIT);
    }

    // Normalize: backend may return { data, meta } or { data, total, page, totalPages }
    let records = response.data || response.records || response || [];

    // Client-side search filter (name search — no backend search endpoint assumed)
    if (search) {
    ...
    const total = meta.total ?? meta.totalCount ?? response.total ?? records.length;
    const pages = meta.totalPages ?? meta.last_page ?? response.totalPages ??
