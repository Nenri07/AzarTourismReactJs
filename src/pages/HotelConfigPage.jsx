...
// const CURRENCIES = ['EUR', 'TRY', 'EGP', 'USD', 'AED', 'GBP', 'SAR', 'QAR', 'OMR', 'KWD', 'BHD', 'MYR', 'TND'];
// const COUNTRIES = ['Turkey', 'Egypt', 'Tunis', 'Malaysia', 'London', 'Germany', 'Qatar', 'Bahrain', 'Oman', 'Kuwait', 'UK'];
...
import React, { useState, useEffect, useCallback } from 'react';
import { Save, Edit2, Trash2, Download, AlertCircle, CheckCircle, Search, X } from 'lucide-react';
import { getHotelConfigs, createHotelConfig, updateHotelConfig, deleteHotelConfig } from '../Api/hotelConfig.api';
...
const HotelConfigPage = () => {
  ...
  const loadConfigs = useCallback(async (page = 1, search = '', country = '') => {
    try {
      setLoading(true);
      setError('');
      ...
      if (country) {
        // Use by-country endpoint when country filter is active
        response = await fetch(`/api/hotel-info/by-country?country=${encodeURIComponent(country)}`);
        response = await response.json();
        // by-country returns all matching — paginate client-side
        const all = response.data || response || [];
        const filtered = search
          ? all.filter(h => h.hotel_name?.toLowerCase().includes(search.toLowerCase()))
          : all;
        const start = (page - 1) * LIMIT;
        setConfigs(filtered.slice(start, start + LIMIT));
        setTotalCount(filtered.length);
        setTotalPages(Math.max(1, Math.ceil(filtered.length / LIMIT)));
      } else {
        // Use paginated endpoint
        response = await fetch(`/api/hotel-info/paginated?page=${page}&limit=${LIMIT}`);
        response = await response.json();
        let all = response.data || response?.records || response || [];
        if (search) {
          all = all.filter(h => h.hotel_name?.toLowerCase().includes(search.toLowerCase()));
        }
        setConfigs(all);
        // Use meta if backend provides it, else estimate
        const meta = response.meta || response.pagination || {};
        setTotalPages(meta.totalPages || meta.last_page || Math.max(1, Math.ceil((meta.total || all.length) / LIMIT)));
        setTotalCount(meta.total || meta.totalCount || all.length);
      }
    } catch (err) {
      // Fallback to getHotelConfigs() if direct fetch fails
      try {
        const fallback = await getHotelConfigs();
        const all = fallback.data || [];
        const filtered = search
          ? all.filter(h => h.hotel_name?.toLowerCase().includes(search.toLowerCase()))
          : country
          ? all.filter(h => h.country?.toLowerCase() === country.toLowerCase())
          : all;
        const start = (page - 1) * LIMIT;
        setConfigs(filtered.slice(start, start + LIMIT));
        setTotalCount(filtered.length);
        ...
        setError('Failed to load configurations: ' + fallbackErr.message);
        setConfigs([]);
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    loadConfigs(currentPage, searchTerm, filterCountry);
  ...
  const handlePageChange = (page) => {
    ...
    setCurrentPage(page);
    window.scrollTo({ top: document.getElementById('configs-list')?.offsetTop - 20 || 0, behavior: 'smooth' });
  };
  ...
  const validateJSON = (jsonString) => {
    try {
      const parsed = JSON.parse(jsonString);
      if (!parsed.hotel_name || !parsed.currency || !parsed.country) return 'Missing hotel_name, currency or country';
      if (!Array.isArray(parsed.form_fields)) return 'form_fields must be an array';
      if (!parsed.conditional_sections || typeof parsed.conditional_sections !== 'object') return 'Invalid conditional_sections';
      if (!parsed.final_calculations || typeof parsed.final_calculations !== 'object') return 'Invalid final_calculations';
      return null;
    } catch (e) {
      return `Invalid JSON: ${e.message}`;
    }
  ...
  const handleSave = async () => {
    ...
    if (!jsonConfig.trim()) { setError('JSON configuration is required'); return; }
    const validationError = validateJSON(jsonConfig);
    ...
    try {
      ...
      await loadConfigs(currentPage, searchTerm, filterCountry);
      resetForm();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
      setError(err.message || 'Failed to save configuration');
  ...
  const handleEdit = useCallback((config) => {
    if (!config.id) { setError('Cannot edit: Invalid configuration ID'); return; }
    setEditingId(config.id);
    setHotelName(config.hotel_name);
    setCurrency(config.currency);
    setCountry(config.country || 'Turkey');
  ...
  const handleDelete = async (config) => {
    if (!config.id) { setError('Cannot delete: Invalid configuration ID'); return; }
    if (!window.confirm(`Delete configuration for "${config.hotel_name}"?`)) return;
    try {
      setError('');
      await deleteHotelConfig(config.id);
      setSuccess('Configuration deleted successfully');
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Hotel Configuration Management</h1>
          <p className="text-slate-600">Configure hotel invoice templates with JSON</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-8">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Hotel Name <span className="text-red-500">*</span></label>
              <input type="text" value={hotelName} onChange={handleHotelNameChange} placeholder="e.g., Novotel Tunis Lac"
                className="w-full h-12 px-4 text-base text-slate-900 bg-white border-2 border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Currency <span className="text-red-500">*</span></label>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-slate-700">JSON Configuration <span className="text-red-500">*</span></label>
                <button onClick={loadSampleJSON} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-slate-100 border-2 border-slate-300 rounded-lg hover:bg-slate-200 transition-colors">
                  <Download className="w-4 h-4" /> Load Sample
                </button>
              </div>
              <textarea value={jsonConfig} onChange={(e) => setJsonConfig(e.target.value)}
                placeholder="Paste your hotel configuration JSON here..." rows={20}
                className="w-full min-h-[500px] p-4 text-sm text-slate-900 bg-white border-2 border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 font-mono leading-relaxed" />
            </div>
            <div className="flex gap-3 pt-4">
              <button onClick={handleSave} disabled={saving}
                ...
                onMouseEnter={(e) => !saving && (e.currentTarget.style.backgroundColor = '#001a3c')}
                onMouseLeave={(e) => !saving && (e.currentTarget.style.backgroundColor = '#002a5c')}>
                {saving ? (<><div className="w-5 h-5 border-3 border-white border-t-transparent rounded-full animate-spin"></div>Saving...</>) : (<><Save className="w-5 h-5" />{editingId ? 'Update' : 'Save'}</>)}
              </button>
              {editingId && (
                <button onClick={resetForm} className="flex items-center gap-2 px-6 py-3 text-base font-medium text-slate-700 bg-slate-200 border-2 border-slate-300 rounded-lg hover:bg-slate-300 transition-colors" style={{ minHeight: '48px' }}>
                  Cancel
                </button>
              )}
            </div>
        ...
        {/* ── Saved Configs with search + pagination ── */}
        <div id="configs-list" className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h2 className="text-xl font-semibold text-slate-800">
              Saved Configurations ({totalCount})
            </h2>
            ...
            <div className="flex flex-wrap gap-2 items-center">
              <select
                ...
                className="h-10 px-3 text-sm text-slate-700 bg-white border-2 border-slate-300 rounded-lg focus:outline-none focus:border-blue-500"
              >
              ...
              <div className="flex items-center gap-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-10 pl-9 pr-3 text-sm text-slate-700 bg-white border-2 border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 w-52"
                  />
                </div>
                <button onClick={handleSearch}
                  className="h-10 px-4 text-sm font-medium text-white rounded-lg transition-colors"
                  style={{ backgroundColor: '#003d7a' }}
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#002a5c'}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#003d7a'}>
                  Search
                ...
                {(searchTerm || filterCountry) && (
                  <button onClick={handleClearSearch} className="h-10 w-10 flex items-center justify-center text-slate-500 hover:text-red-500 border-2 border-slate-300 rounded-lg hover:border-red-300 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                )}
              </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-12 h-12 border-4 border-slate-300 border-t-4 rounded-full animate-spin" style={{ borderTopColor: '#002a5c' }}></div>
            </div>
          ) : configs.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-slate-500 mb-4">No configurations found</p>
              <p className="text-sm text-slate-400">{searchTerm || filterCountry ? 'Try adjusting your search' : 'Create your first configuration above'}</p>
            </div>
          ) : (
            <>
              <div className="space-y-4 mb-6">
                {configs.map((config) => (
                  <div key={config.id} className="border border-slate-200 rounded-lg p-4 hover:border-slate-300 transition-colors">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-slate-800 mb-1">{config.hotel_name}</h3>
                        <div className="flex items-center gap-4 text-sm text-slate-500">
                          <span>Currency: <span className="font-medium text-slate-700">{config.currency}</span></span>
                          <span>Country: <span className="font-medium text-slate-700">{config.country}</span></span>
                          <span>ID: <span className="font-mono font-medium text-slate-700">{config.id}</span></span>
                          {config.created_at && <span>Created: {new Date(config.created_at).toLocaleDateString()}</span>}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                    ...
                    {getPageNumbers().map((page, idx) =>
                      page === '...' ? (
                        ...
                        <button
                          ...
                          style={currentPage === page ? { backgroundColor: '#002a5c', borderColor: '#002a5c', color: 'white' } : {}}
