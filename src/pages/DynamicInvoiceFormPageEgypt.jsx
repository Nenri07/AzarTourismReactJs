...
export default function DynamicInvoiceFormPageEgypt() {
  ...
  useEffect(() => {
    ...
    // Check if Departure is before or exactly the same as Arrival
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Set the data so the modal knows what to display
      setSavedInvoiceData({
        isEdit: isEditMode,
      ...
      // FIXED: Show the modal, DO NOT navigate away automatically. 
      // The user must click close on the modal to navigate.
      setTimeout(() => {
        const modalElement = document.getElementById('success_modal');
        if (modalElement) {
          modalElement.showModal();
        } else {
          // Fallback just in case the modal element fails to mount
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-32">
      <div className="max-w-7xl mx-auto p-3 sm:p-4 md:p-6 font-sans text-slate-800">
        ...
        {/* FIXED: Date Error Banner properly added back to the UI */}
        {dateError && (
      ...
      {/* FIXED: The Success Modal is now correctly rendered so it can pop up! */}
      {savedInvoiceData && (
        <SuccessModal
          ...
          onClose={() => navigate("/invoices")} // Navigates ONLY when they close the modal
        />
      )}
    </div>
  );
